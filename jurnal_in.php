...
if (isset($_SESSION['user_id']))
    {
        ...
        $query = "SHOW TABLES LIKE \"DB_".$_SESSION['user_year']."_IN\";";
        $res = mysql_query($query) or die(mysql_error());
        $queryes_num++;
        if (mysql_num_rows($res) == 0)
            {
                if (@opendir("uploads/".$_SESSION['user_year']."/IN"))
                    {
                        @closedir("uploads/".$_SESSION['user_year']."/IN");
                    }
                    else
                    {
                        mkdir("uploads/".$_SESSION['user_year']."/IN", null, true) or die("Can't create new folder - uploads/".$_SESSION['user_year']."/IN");
                    }
                                        ...
                                        if (isset($_POST['get_data']) and !empty($_POST['get_data']))
                                            {
                                                ...
                                                if ($error == "")
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        $page = str_replace("{INFORMATION}", "<a onclick=\"skm_LockScreen()\" href=\"jurnal_in.php?edit=".$_GET['edit']."\">{LANG_RETURN_AND_GO}</a>", $page);
        ...
        if (isset($_GET['do']) AND $_GET['do'] == "made" AND isset($_GET['id']) AND preg_match("/^[1-9][0-9]*$/", $_GET['id']))
            {
                $query = "SELECT * FROM `db_".$_SESSION['user_year']."_in` WHERE `id`='".$_GET['id']."' LIMIT 1 ; ";
                $res = mysql_query($query) or die(mysql_error());
                $queryes_num++;
                if (mysql_num_rows($res) == 1)
                    {
                        while ($row=mysql_fetch_array($res))
                            {
                                $error = "";
                                if (!empty($row['do_made'])) $error = "{LANG_NEW_OUT_WITH_DO_MADED}<br>";
                                if ($row['do_user'] != $_SESSION['user_id'] AND $user_p_mod != 1) $error = "{LANG_NEW_OUT_WITH_NOT_DO_USER}<br>";
                                if ($error == "")
                                    {
                                        $query = "UPDATE `db_".$_SESSION['user_year']."_in` SET `do_made`='".date('Y-m-d H:i:s')."', `do_made_ip`='".$_SERVER['REMOTE_ADDR']."' WHERE `id`='".$row['id']."' LIMIT 1 ; ";
                                        mysql_query($query) or die(mysql_error());
                                        $queryes_num++;
                                        $page.= file_get_contents("templates/information.html");
                                        $page = str_replace("{INFORMATION}", "{LANG_NEW_OUT_WITH_UPDATED}: <strong>".$row['id']."</strong><br><kbd>{LANG_JURNAL_IN_STATUS_3} ".date('d.m.Y H:i:s')."</kbd>", $page);
                                    }
                                    else
                                    {
                                        $adres = 'true';
        ...
        if ($adres <> 'true')
            {
                ...
                if (isset($where_lang) AND !empty($where_lang))
                    {
                        $page.= file_get_contents("templates/information.html");
                        $page = str_replace("{INFORMATION}", $where_lang." <a onclick=\"skm_LockScreen()\" class=\"btn btn-default btn-sm\" href=\"jurnal_in.php\">{LANG_CLEAN_SERCH_RESULTS}</a>", $page);
                    }
                ...
                $html_navy = get_navy("DB_".$_SESSION['user_year']."_IN", $query_where, $query_order_by, $active, $_SESSION['user_page_limit'], "jurnal_in.php?".$pre_link."&page_num=");
                                ...
                                if ($row['do_user'] == $_SESSION['user_id'] OR $user_p_mod == 1)
                                    {
                                        ...
                                        $admin_links_do .= "<a onclick=\"skm_LockScreen()\" href=\"jurnal_out.php?add=do&from=in&id=".$row['id']."\" class=\"btn btn-success btn-lg\" role=\"button\"><span class=\"glyphicon glyphicon-send\" aria-hidden=\"true\" data-toggle=\"tooltip\" data-original-title=\"{LANG_NEW_OUT_WITH_IN_ID}\"></span></a>";
                                    }
                                if ($privat4 == 1) $admin_links_do .= "<a onclick=\"skm_LockScreen()\" href=\"?do=add&template=".$row['id']."\" class=\"btn btn-info btn-lg\" role=\"button\"><span class=\"glyphicon glyphicon-random\" aria-hidden=\"true\" data-toggle=\"tooltip\" data-original-title=\"{LANG_NEW_WITH_TEMPLATE}\"></span></a>";
                                $show_files = 0;
